...
const countries = [
  { name: "Singapore", w: 46 }, { name: "Switzerland", w: 46 },
  { name: "Colombia", w: 25 }, { name: "Norway", w: 21 },
  { name: "Poland", w: 13 }, { name: "Germany", w: 10 },
  { name: "Sweden", w: 8 }, { name: "Latvia", w: 7 },
  { name: "Kenya", w: 6 }, { name: "Austria", w: 5 },
  { name: "Finland", w: 4 }, { name: "Ukraine", w: 3 },
  { name: "Spain", w: 2 }, { name: "Romania", w: 2 },
  { name: "Denmark", w: 1 }, { name: "Vietnam", w: 1 },
  { name: "Georgia", w: 1 }, { name: "Cyprus", w: 1 },
  { name: "Burundi", w: 1 }, { name: "Rwanda", w: 1 },
  { name: "Zimbabwe", w: 1 }, { name: "Zambia", w: 1 },
];

const technologies = [
  { name: "Solar PV", w: 142 }, { name: "BESS", w: 40 },
  { name: "Hydro", w: 15 }, { name: "Waste to Energy", w: 3 },
  { name: "Biomass", w: 2 }, { name: "Geothermal", w: 3 },
  { name: "Wind", w: 1 },
];

const statuses = [
  { name: "Operational", w: 77 }, { name: "Ready to Build", w: 75 },
  { name: "Development", w: 48 }, { name: "Construction", w: 6 },
];

const currencies: Record<string, string> = {
  Singapore: "SGD", Switzerland: "CHF", Colombia: "COP",
  Norway: "NOK", Poland: "PLN", Kenya: "USD",
  Burundi: "USD", Rwanda: "USD", Zimbabwe: "USD", Zambia: "USD",
};
const defaultCurrency = "EUR";

const codYears = [
  { year: 2019, w: 2 }, { year: 2021, w: 2 }, { year: 2022, w: 1 },
  { year: 2023, w: 29 }, { year: 2024, w: 29 }, { year: 2025, w: 29 },
  { year: 2026, w: 20 }, { year: 2027, w: 12 }, { year: 2028, w: 5 },
  { year: 2029, w: 1 }, { year: 2030, w: 2 },
];

const financingTypes = [
  { name: "Debt", w: 20 }, { name: "EquityAndDebt", w: 8 }, { name: "Equity", w: 6 },
];

const friggDescMap: Record<string, [number, number]> = {
  "Excellent": [8, 10], "Good": [6, 8], "Fair": [4, 6],
  "Poor": [2, 4], "Very Poor": [0, 2],
};
const friggDescs = [
  { name: "Very Poor", w: 91 }, { name: "Excellent", w: 20 },
  { name: "Poor", w: 13 }, { name: "Fair", w: 7 }, { name: "Good", w: 5 },
];

// Fictional project name parts
const prefixes = [
  "Aurora", "Beacon", "Cascade", "Delta", "Echo", "Falcon", "Glacier",
  "Horizon", "Ivory", "Jade", "Kodiak", "Lumen", "Mesa", "Nova",
  "Orbit", "Pinnacle", "Quartz", "Ridge", "Summit", "Terra",
  "Unity", "Vortex", "Zephyr", "Alpine", "Breeze", "Coral", "Dune",
  "Ember", "Fjord", "Grove", "Helix", "Iris", "Kelp", "Lotus",
  "Maple", "Nexus", "Oasis", "Prism", "Reef", "Sage", "Tidal",
];
const suffixes = [
  "Energy", "Power", "Solar", "Green", "Renewables", "Solutions",
  "Systems", "Partners", "Ventures", "Grid", "Labs", "Works",
  "Holdings", "Capital", "Industries", "Tech", "Corp", "Group",
];
const qualifiers = [
  "I", "II", "III", "Alpha", "Beta", "North", "South", "East", "West",
  "Prime", "Plus", "One", "Max", "Pro", "Flex", "Core",
];
...
for (let i = 0; i < N; i++) {
  const country = pick(countries.map(c => c.name), countries.map(c => c.w));
  const tech = pick(technologies.map(t => t.name), technologies.map(t => t.w));
  const status = pick(statuses.map(s => s.name), statuses.map(s => s.w));
  const currency = currencies[country] ?? defaultCurrency;
  const codYear = pick(codYears.map(y => y.year), codYears.map(y => y.w));
  ...
  const friggDesc = hasFrigg ? pick(friggDescs.map(f => f.name), friggDescs.map(f => f.w)) : null;
  const friggRange = friggDesc ? friggDescMap[friggDesc] : null;
  const friggScore = friggRange ? roundTo(rand(friggRange[0], friggRange[1]), 1) : 0;
  ...
  const financingType = hasFinancing
    ? pick(financingTypes.map(f => f.name), financingTypes.map(f => f.w))
    : "Debt";
  const financingCloseDate = hasFinancing ? genDate(2025 + Math.floor(Math.random() * 2)) : null;
  ...
  const createdAt = genDate(createdYear) + `T${String(Math.floor(rand(6, 22))).padStart(2, "0")}:${String(Math.floor(rand(0, 60))).padStart(2, "0")}:${String(Math.floor(rand(0, 60))).padStart(2, "0")}.000Z`;
  ...
  projects.push({
    ...
    "Capacity Payments % (BESS)": tech === "BESS" ? roundTo(rand(0.3, 1), 2) : 0,
    ...
    "Suggested Additional Leverage": hasFinancing ? roundTo(logNorm(500000, 1), 2) : 0,
...
console.log(`Generated ${projects.length} dummy projects → src/lib/projects.json`);
